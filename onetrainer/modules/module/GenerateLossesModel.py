...
from onetrainer.modules.dataLoader import StableDiffusionFineTuneDataLoader
...
class GenerateLossesModel:
    ...
    def start(self):
        ...
        self.model_loader = create.create_model_loader(
            self.config.model_type, self.config.training_method)
        ...
        with torch.inference_mode():
            for epoch_step, batch in enumerate(step_tqdm):
                ...
                filename_loss_list.append(
                    (batch['image_path'][0], float(loss)))

        # Sort such that highest loss comes first
        filename_loss_list.sort(key=lambda x: x[1], reverse=True)
        filename_to_loss: dict[str, float] = {
            x[0]: x[1] for x in filename_loss_list}
