...
class ModelSetupDiffusionNoiseMixin(metaclass=ABCMeta):
    ...
    def _create_noise(
        ...
        if config.perturbation_noise_weight > 0:
            ...
            noise = noise + \
                (config.perturbation_noise_weight * perturbation_noise)
    ...
    def _get_timestep_discrete(
        ...
        if not deterministic:
            min_timestep = int(
                noise_scheduler.config['num_train_timesteps'] * config.min_noising_strength)
            max_timestep = int(
                noise_scheduler.config['num_train_timesteps'] * config.max_noising_strength)
            if config.noising_weight == 0:
                ...
                weights = 1 / (1 + np.exp(-config.noising_weight *
                               (weights - config.noising_bias)))  # Sigmoid
                weights /= np.sum(weights)
                samples = rng.choice(
                    np.arange(min_timestep, max_timestep), size=(batch_size,), p=weights)
    ...
    def _get_timestep_continuous(
        ...
        if not deterministic:
            if config.noising_weight == 0:
                ...
                rng = np.random.default_rng(global_step)
                # Discretize range (0, 1]
                choices = np.linspace(np.finfo(float).eps, 1, 5000)
                weights = 1 / (1 + np.exp(-config.noising_weight *
                               (choices - config.noising_bias)))  # Sigmoid
                weights /= np.sum(weights)
                samples = rng.choice(choices, size=(batch_size,), p=weights)
                samples = samples * (config.max_noising_strength -
                                     config.min_noising_strength) + config.min_noising_strength
    ...
    def _add_noise_discrete(
        ...
        if self.__coefficients is None:
            ...
            self.__coefficients = DiffusionScheduleCoefficients.from_betas(
                betas)

        orig_dtype = scaled_latent_image.dtype

        sqrt_alphas_cumprod = self.__coefficients.sqrt_alphas_cumprod[timestep]
        sqrt_one_minus_alphas_cumprod = self.__coefficients.sqrt_one_minus_alphas_cumprod[
            timestep]
        ...
        scaled_noisy_latent_image = scaled_latent_image.to(dtype=sqrt_alphas_cumprod.dtype) * sqrt_alphas_cumprod \
            + latent_noise.to(dtype=sqrt_alphas_cumprod.dtype) * \
    ...
    def _add_noise_continuous(
        ...
        alphas_cumprod = alphas_cumprod_fun(
            timestep, scaled_latent_image.dim())
