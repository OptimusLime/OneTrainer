...
from onetrainer.modules.module.AestheticScoreModel import AestheticScoreModel
from onetrainer.modules.module.HPSv2ScoreModel import HPSv2ScoreModel
...
from onetrainer.modules.util.loss.masked_loss import masked_losses
from onetrainer.modules.util.loss.vb_loss import vb_losses
...
class ModelSetupDiffusionLossMixin(metaclass=ABCMeta):
    ...
    def __unmasked_losses(
        ...
        if config.masked_training and config.normalize_masked_area_loss:
            clamped_mask = torch.clamp(
                batch['latent_mask'], config.unmasked_weight, 1)
    ...
    def __min_snr_weight(
        ...
        if self.__coefficients:
            ...
            snr = ((alphas_cumprod ** 0.5) /
                   (1.0 - alphas_cumprod) ** 0.5) ** 2
    ...
    def _diffusion_losses(
        ...
        if data['loss_type'] == 'align_prop':
            losses = self.__align_prop_losses(
                batch, data, config, train_device)
        else:
        ...
        if config.min_snr_gamma and 'timestep' in data and not data['loss_type'] == 'align_prop':
            ...
            snr_weight = self.__min_snr_weight(
                data['timestep'], config.min_snr_gamma, v_pred, losses.device)
            losses *= snr_weight
