...
from onetrainer.modules.model.PixArtAlphaModel import PixArtAlphaModel
...
class PixArtAlphaBaseDataLoader(BaseDataLoader):
    ...
    def _load_input_modules(self, config: TrainConfig, model: PixArtAlphaModel) -> list:
    ...
    def _preparation_modules(self, config: TrainConfig, model: PixArtAlphaModel):
        ...
        encode_image = EncodeVAE(in_name='image', out_name='latent_image_distribution',
                                 vae=model.vae, autocast_context=model.autocast_context)
        downscale_mask = ScaleImage(
            in_name='mask', out_name='latent_mask', factor=0.125)
        encode_conditioning_image = EncodeVAE(
            in_name='conditioning_image', out_name='latent_conditioning_image_distribution', vae=model.vae, autocast_context=model.autocast_context)
        tokenize_prompt = Tokenize(in_name='prompt', tokens_out_name='tokens',
                                   mask_out_name='tokens_mask', tokenizer=model.tokenizer, max_token_length=120)
        encode_prompt = EncodeT5Text(tokens_in_name='tokens', tokens_attention_mask_in_name='tokens_mask', hidden_state_out_name='text_encoder_hidden_state', pooled_out_name=None,
                                     add_layer_norm=True, text_encoder=model.text_encoder, hidden_state_output_index=-(1 + config.text_encoder_layer_skip), autocast_context=model.text_encoder_autocast_context)
    ...
    def _cache_modules(self, config: TrainConfig, model: PixArtAlphaModel):
        ...
        text_split_names = ['tokens', 'tokens_mask',
    ...
    def _output_modules(self, config: TrainConfig, model: PixArtAlphaModel):
        output_names = ['latent_image', 'tokens',
                        'tokens_mask', 'image_path', 'prompt']
    ...
    def _debug_modules(self, config: TrainConfig, model: PixArtAlphaModel):
        ...
        decode_conditioning_image = DecodeVAE(
            in_name='latent_conditioning_image', out_name='decoded_conditioning_image', vae=model.vae, autocast_context=model.autocast_context)
        upscale_mask = ScaleImage(
            in_name='latent_mask', out_name='decoded_mask', factor=8)
        decode_prompt = DecodeTokens(
            in_name='tokens', out_name='decoded_prompt', tokenizer=model.tokenizer)
