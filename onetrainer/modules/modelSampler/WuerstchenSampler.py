...
class WuerstchenSampler(BaseModelSampler):
    ...
    def __sample_prior(
        ...
        tokens_attention_mask = tokenizer_output.attention_mask.to(
            self.train_device)
        ...
        negative_tokens = negative_tokenizer_output.input_ids.to(
            self.train_device)
        negative_tokens_attention_mask = negative_tokenizer_output.attention_mask.to(
            self.train_device)
        ...
        with self.model.autocast_context:
            ...
            if self.model_type.is_wuerstchen_v2():
                ...
                prompt_embedding = final_layer_norm(
                    text_encoder_output.hidden_states[-(
                        1 + text_encoder_layer_skip)]
                )
            elif self.model_type.is_stable_cascade():
                prompt_embedding = text_encoder_output.hidden_states[-(
                    1 + text_encoder_layer_skip)]
                pooled_prompt_embedding = text_encoder_output.text_embeds.unsqueeze(
                    1)
            ...
            if self.model_type.is_wuerstchen_v2():
                ...
                negative_prompt_embedding = final_layer_norm(
                    negative_text_encoder_output.hidden_states[-(
                        1 + text_encoder_layer_skip)]
                )
            if self.model_type.is_stable_cascade():
                negative_prompt_embedding = negative_text_encoder_output.hidden_states[-(
                    1 + text_encoder_layer_skip)]
                pooled_negative_prompt_embedding = negative_text_encoder_output.text_embeds.unsqueeze(
                    1)
        ...
        # prepare timesteps
        prior_noise_scheduler.set_timesteps(
        ...
        clip_img = torch.zeros(size=(
            2, 1, 768), dtype=combined_prompt_embedding.dtype, device=combined_prompt_embedding.device)
    ...
    def __sample_decoder(
        ...
        with self.model.autocast_context:
            ...
            prompt_embedding = final_layer_norm(
                text_encoder_output.hidden_states[-(1 +
                                                    text_encoder_layer_skip)]
            )
    ...
    def __sample_base(
        ...
        with self.model.autocast_context:
            ...
            image_array = image_tensor.permute(
                0, 2, 3, 1).cpu().squeeze().float().numpy()
