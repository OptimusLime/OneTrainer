...
class StablDiffusionBaseDataLoader(BaseDataLoader):
    ...
    def _load_input_modules(self, config: TrainConfig, model: StableDiffusionModel) -> list:
        ...
        generate_depth = GenerateDepth(path_in_name='image_path', image_out_name='depth',
                                       image_depth_processor=model.image_depth_processor, depth_estimator=model.depth_estimator)
    ...
    def _crop_modules(self, config: TrainConfig):
        ...
        scale_crop_depth = ScaleCropImage(image_in_name='depth', scale_resolution_in_name='scale_resolution', crop_resolution_in_name='crop_resolution',
                                          enable_crop_jitter_in_name='concept.enable_crop_jitter', image_out_name='depth', crop_offset_out_name='crop_offset')
    ...
    def _preparation_modules(self, config: TrainConfig, model: StableDiffusionModel):
        ...
        encode_conditioning_image = EncodeVAE(
            in_name='conditioning_image', out_name='latent_conditioning_image_distribution', vae=model.vae)
        downscale_depth = ScaleImage(
            in_name='depth', out_name='latent_depth', factor=0.125)
        tokenize_prompt = Tokenize(in_name='prompt', tokens_out_name='tokens', mask_out_name='tokens_mask',
                                   tokenizer=model.tokenizer, max_token_length=model.tokenizer.model_max_length)
        encode_prompt = EncodeClipText(in_name='tokens', tokens_attention_mask_in_name=None, hidden_state_out_name='text_encoder_hidden_state', pooled_out_name=None,
                                       add_layer_norm=True, text_encoder=model.text_encoder, hidden_state_output_index=-(1 + config.text_encoder_layer_skip), autocast_context=model.autocast_context)
