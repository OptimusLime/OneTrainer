...
from onetrainer.modules.ui.SampleFrame import SampleFrame
...
class SampleWindow(ctk.CTkToplevel):
    def __init__(
        ...
        prompt_frame = SampleFrame(
            self, self.sample, self.ui_state, include_settings=False)
        prompt_frame.grid(row=0, column=0, columnspan=2,
                          padx=0, pady=0, sticky="nsew")

        settings_frame = SampleFrame(
            self, self.sample, self.ui_state, include_prompt=False)
        ...
        image_label = ctk.CTkLabel(
            master=self, text="", image=self.image, height=512, width=512)
