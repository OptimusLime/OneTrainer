...
from onetrainer.modules.util.ui.ToolTip import ToolTip
...
def app_title(master, row, column):
    ...
    image_component = ctk.CTkImage(
        Image.open("resources/icons/icon.png").resize((40, 40),
    ...
    label_component = ctk.CTkLabel(
        frame, text="OneTrainer", font=ctk.CTkFont(size=20, weight="bold"))
        ...
        def destroy(self):
            ...
            if self._textvariable_callback_name:
                self._textvariable.trace_remove(
                    "write", self._textvariable_callback_name)
...
def file_entry(
    ...
    entry_component = ctk.CTkEntry(
        frame, textvariable=ui_state.get_var(var_name))
    entry_component.grid(row=0, column=0, padx=(
        PAD, PAD), pady=PAD, sticky="new")
    ...
    button_component = ctk.CTkButton(
        frame, text="...", width=40, command=__open_dialog)
    button_component.grid(row=0, column=1, padx=(0, PAD),
                          pady=PAD, sticky="nsew")
...
def time_entry(master, row, column, ui_state: UIState, var_name: str, unit_var_name, supports_time_units: bool = True):
    ...
    entry_component = ctk.CTkEntry(
        frame, textvariable=ui_state.get_var(var_name), width=50)
...
def options(master, row, column, values, ui_state: UIState, var_name: str, command: Callable[[str], None] = None):
    component = ctk.CTkOptionMenu(
        master, values=values, variable=ui_state.get_var(var_name), command=command)
    component.grid(row=row, column=column, padx=PAD,
                   pady=(PAD, PAD), sticky="new")
    ...
    component._dropdown_menu.destroy = lambda: destroy(
        component._dropdown_menu)
...
def options_adv(master, row, column, values, ui_state: UIState, var_name: str,
    ...
    component = ctk.CTkOptionMenu(
        frame, values=values, variable=ui_state.get_var(var_name), command=command)
    ...
    if command:
        # call command once to set the initial value
...
def options_kv(master, row, column, values: list[Tuple[str, Any]], ui_state: UIState, var_name: str,
    ...
    component = ctk.CTkOptionMenu(
        master, values=keys, command=update_component)
    component.grid(row=row, column=column, padx=PAD,
                   pady=(PAD, PAD), sticky="new")
...
def double_progress(master, row, column, label_1, label_2):
    ...
    label_1_component.grid(row=0, column=0, padx=(
        PAD, PAD), pady=(0, 0), sticky="new")

    label_2_component = ctk.CTkLabel(frame, text=label_2)
    label_2_component.grid(row=1, column=0, padx=(
        PAD, PAD), pady=(0, 0), sticky="sew")

    progress_1_component = ctk.CTkProgressBar(frame)
    progress_1_component.grid(row=0, column=1, padx=(
        PAD, PAD), pady=(PAD, 0), sticky="new")

    progress_2_component = ctk.CTkProgressBar(frame)
    progress_2_component.grid(row=1, column=1, padx=(
        PAD, PAD), pady=(0, PAD), sticky="sew")

    description_1_component = ctk.CTkLabel(frame, text="")
    description_1_component.grid(row=0, column=2, padx=(
        PAD, PAD), pady=(0, 0), sticky="new")

    description_2_component = ctk.CTkLabel(frame, text="")
    description_2_component.grid(row=1, column=2, padx=(
        PAD, PAD), pady=(0, 0), sticky="sew")

    def set_1(value, max_value):
        progress_1_component.set(value / max_value)
        description_1_component.configure(
            text="{0}/{1}".format(value, max_value))

    def set_2(value, max_value):
        progress_2_component.set(value / max_value)
        description_2_component.configure(
            text="{0}/{1}".format(value, max_value))
