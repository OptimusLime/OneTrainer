...
class StableDiffusionFineTuneVaeSetup(BaseStableDiffusionSetup):
    ...
    def predict(
        ...
        predicted_image = model.vae.decode(
            latent_image, return_dict=True).sample
        ...
        if config.debug_mode:
            with torch.no_grad():
                # image
                self._save_image(
                    image, config.debug_dir + "/training_batches", "1-image", train_progress.global_step)

                # predicted image
                predicted_image_clamped = predicted_image.clamp(-1, 1)
                self._save_image(
                    predicted_image_clamped, config.debug_dir +
                    "/training_batches", "2-predicted_image",
