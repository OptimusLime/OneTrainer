...
from onetrainer.modules.ui.GenerateCaptionsWindow import GenerateCaptionsWindow
from onetrainer.modules.ui.GenerateMasksWindow import GenerateMasksWindow
...
Mouse wheel: increase or decrease brush size"""
    ...
    def content_column(self, master):
        ...
        enable_mask_editing_checkbox.grid(
            row=0, column=2, padx=25, pady=5, sticky="w")
        ...
        mask_editing_alpha_label = ctk.CTkLabel(
            right_frame, text="Brush Alpha", width=75)
    ...
    def switch_image(self, index):
        ...
        if index >= 0:
            ...
            scale = self.image_size / \
                max(self.pil_image.height, self.pil_image.width)
        ...
        if self.pil_mask:
            ...
            if self.display_only_mask:
                self.image.configure(
                    light_image=resized_pil_mask, size=resized_pil_mask.size)
            else:
                ...
                if np_mask_min == 0:
                    ...
                    np_mask = (np_mask - np_mask_min) / \
                        (1.0 - np_mask_min) * (1.0 - norm_min) + norm_min

                np_masked_image = (np_image * np_mask * 255.0).astype(np.uint8)
                masked_image = Image.fromarray(np_masked_image, mode='RGB')

                self.image.configure(
                    light_image=masked_image, size=masked_image.size)
        else:
            self.image.configure(light_image=self.pil_image,
    ...
    def edit_mask(self, event):
        ...
        end_y = int(self.mask_draw_y /
                    self.pil_image.height * self.image_height)
    ...
    def draw_mask(self, start_x, start_y, end_x, end_y, is_left, is_right):
        ...
        if is_left:
            ...
            rgb_value = int(max(0.0, min(alpha, 1.0)) * 255)
        ...
        if color is not None:
            if self.pil_mask is None:
                if adding_to_mask:
                    self.pil_mask = Image.new('RGB', size=(
                        self.image_width, self.image_height), color=(0, 0, 0))
                else:
                    self.pil_mask = Image.new('RGB', size=(
                        self.image_width, self.image_height), color=(255, 255, 255))

            radius = int(self.mask_draw_radius *
    ...
    def open_directory(self):
        ...
        if new_dir:
            self.dir = new_dir
            self.load_directory(
                include_subdirectories=self.config_ui_data["include_subdirectories"])
