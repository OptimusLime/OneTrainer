...
from onetrainer.modules.modelSetup.stableDiffusion.checkpointing_util import \
...
class BaseStableDiffusionXLSetup(
    ...
    def setup_optimizations(
        ...
        if config.gradient_checkpointing:
            ...
            enable_checkpointing_for_transformer_blocks(
                model.unet, self.train_device)
            enable_checkpointing_for_clip_encoder_layers(
                model.text_encoder_1, self.train_device)
            enable_checkpointing_for_clip_encoder_layers(
                model.text_encoder_2, self.train_device)
    ...
    def __encode_text(
        ...
        if tokens_1 is None and text is not None:
            ...
            tokens_1 = tokenizer_output.input_ids.to(
                model.text_encoder_1.device)
        ...
        if tokens_2 is None and text is not None:
            ...
            tokens_2 = tokenizer_output.input_ids.to(
                model.text_encoder_2.device)
        ...
        if text_encoder_1_output is None:
            ...
            text_encoder_1_output = text_encoder_1_output.hidden_states[-(
                2 + text_encoder_layer_skip)]
        ...
        if text_encoder_2_output is None or pooled_text_encoder_2_output is None:
            ...
            text_encoder_2_output = text_encoder_2_output.hidden_states[-(
                2 + text_encoder_2_layer_skip)]

        text_encoder_output = torch.concat(
    ...
    def predict(
        ...
        with model.autocast_context:
            ...
            if is_align_prop_step and not deterministic:
                ...
                added_cond_kwargs = {
                    "text_embeds": pooled_text_encoder_2_output, "time_ids": add_time_ids}
                negative_added_cond_kwargs = {"text_embeds": negative_pooled_text_encoder_2_output,
                                              "time_ids": add_time_ids}

                checkpointed_unet = create_checkpointed_forward(
                    model.unet, self.train_device)
                ...
                for step in range(config.align_prop_steps):
                    ...
                    if self.debug_mode:
                        with torch.no_grad():
                            # predicted image
                            predicted_image = self._project_latent_to_image_sdxl(
                ...
                if model.noise_scheduler.config.prediction_type == 'epsilon':
                    ...
                    target_velocity = model.noise_scheduler.get_velocity(
            ...
            if config.debug_mode:
                with torch.no_grad():
                    self._save_text(
                        self._decode_tokens(
                            batch['tokens_1'], model.tokenizer_1),
                        config.debug_dir + "/training_batches",
                        "7-prompt",
                        train_progress.global_step,
                    )
                    ...
                    if is_align_prop_step:
                        ...
                        # image
                        self._save_image(
                            self._project_latent_to_image_sdxl(
                                scaled_latent_image),
                            config.debug_dir + "/training_batches",
                            "2-image",
