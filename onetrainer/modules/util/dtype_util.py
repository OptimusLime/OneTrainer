...
def allow_mixed_precision(train_config: TrainConfig):
    all_dtypes = list(train_config.weight_dtypes(
    ).all_dtypes() + [train_config.train_dtype])
...
def enable_grad_scaling(train_dtype: DataType, parameters: list[Parameter]):
    trainable_parameter_dtype = list(
        set([parameter.dtype for parameter in parameters]))
...
def create_autocast_context(
    ...
    weight_dtypes = list(filter(lambda dtype: dtype !=
                         DataType.NONE and dtype is not None, weight_dtypes))
