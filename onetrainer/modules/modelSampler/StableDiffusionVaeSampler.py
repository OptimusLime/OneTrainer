...
class StableDiffusionVaeSampler(BaseModelSampler):
    ...
    def sample(
        ...
        image_tensor = t_in(image).to(
            device=self.train_device, dtype=self.model.vae.dtype)
        image_tensor = image_tensor * 2 - 1

        self.model.vae_to(self.train_device)

        with torch.no_grad():
            latent_image_tensor = self.model.vae.encode(
                image_tensor.unsqueeze(0)).latent_dist.mean
            image_tensor = self.model.vae.decode(
                latent_image_tensor).sample.squeeze()
