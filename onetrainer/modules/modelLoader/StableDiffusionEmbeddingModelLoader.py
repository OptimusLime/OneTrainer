...
class StableDiffusionEmbeddingModelLoader(BaseModelLoader, ModelLoaderModelSpecMixin):
    ...
    def __load_safetensors(
        ...
        model.model_spec = self._load_default_model_spec(
            model.model_type, embedding_name)
    ...
    def __load_internal(
        ...
        # embedding model
        pt_embedding_name = os.path.join(
            embedding_name, "embedding", "embedding.pt")
        safetensors_embedding_name = os.path.join(
            embedding_name, "embedding", "embedding.safetensors")
        ...
        try:
            model.optimizer_state_dict = torch.load(
                os.path.join(embedding_name, "optimizer", "optimizer.pt"))
        ...
        try:
            model.ema_state_dict = torch.load(
                os.path.join(embedding_name, "ema", "ema.pt"))
        except FileNotFoundError:
            pass
    ...
    def load(
        ...
        for stacktrace in stacktraces:
            print(stacktrace)
        raise Exception("could not load embedding: " +
                        str(model_names.embedding))
