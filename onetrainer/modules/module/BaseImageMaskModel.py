...
class BaseImageMaskModel(metaclass=ABCMeta):
    ...
    def mask_images(
        ...
        for i, filename in enumerate(tqdm(filenames)):
            try:
                self.mask_image(filename, prompts, mode, alpha,
                                threshold, smooth_pixels, expand_pixels)
    ...
    def mask_folder(
        ...
        filenames = self.__get_sample_filenames(
            sample_dir, include_subdirectories)
        self.mask_images(
            filenames=filenames,
            prompts=prompts,
            mode=mode,
