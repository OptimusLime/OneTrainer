...
def __q_posterior_mean_variance(
    ...
    posterior_mean = (
        __extract_into_tensor(
            coefficients.posterior_mean_coef1, t, x_t.shape) * x_0
...
def __p_mean_variance(
    ...
    min_log = __extract_into_tensor(
        coefficients.posterior_log_variance_clipped, t, x_t.shape)
    max_log = __extract_into_tensor(
        torch.log(coefficients.betas), t, x_t.shape)
    ...
    predicted_x_0 = __predict_x_0_from_eps(
        coefficients=coefficients, x_t=x_t, t=t, eps=frozen_predicted_eps)
    predicted_mean, _ = __q_posterior_mean_variance(
        coefficients=coefficients, x_0=predicted_x_0, x_t=x_t, t=t)
...
def __predict_x_0_from_eps(
    ...
    return (
        __extract_into_tensor(
            coefficients.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t
