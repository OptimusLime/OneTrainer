...
from onetrainer.modules.modelSetup.BasePixArtAlphaSetup import BasePixArtAlphaSetup
        ...
        if config.text_encoder.train:
            param_groups.append(
                self.create_param_groups(
                    config, model.text_encoder_lora.parameters(), config.text_encoder.learning_rate)
            )

        if config.prior.train:
            param_groups.append(
                self.create_param_groups(
                    config, model.transformer_lora.parameters(), config.prior.learning_rate)
            )
    ...
    def setup_model(
        ...
        if model.transformer_lora is None and config.prior.train:
            model.transformer_lora = LoRAModuleWrapper(
                model.transformer, config.lora_rank, "lora_transformer", config.lora_alpha, [
                    "attn1", "attn2"]
            )
        ...
        if model.transformer_lora is not None:
            train_prior = config.prior.train and \
                not self.stop_prior_training_elapsed(
                    config, model.train_progress)
        ...
        if model.text_encoder_lora is not None:
            ...
            model.text_encoder_lora.to(
                dtype=config.lora_weight_dtype.torch_dtype())
        if model.transformer_lora is not None:
            model.transformer_lora.hook_to_module()
            model.transformer_lora.to(
                dtype=config.lora_weight_dtype.torch_dtype())
