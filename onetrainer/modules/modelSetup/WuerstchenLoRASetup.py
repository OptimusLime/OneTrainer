...
from onetrainer.modules.modelSetup.BaseWuerstchenSetup import BaseWuerstchenSetup
...
class WuerstchenLoRASetup(BaseWuerstchenSetup):
    ...
    def create_parameters_for_optimizer(
        ...
        if config.prior.train:
            param_groups.append(
                self.create_param_groups(
                    config, model.prior_prior_lora.parameters(), config.prior.learning_rate)
            )
    ...
    def setup_model(
        ...
        if model.prior_prior_lora is None and config.prior.train:
            model.prior_prior_lora = LoRAModuleWrapper(
                model.prior_prior, config.lora_rank, "lora_prior_unet", config.lora_alpha, [
                    "attention"]
            )
