...
from onetrainer.modules.modelSetup.mixin.ModelSetupDebugMixin import ModelSetupDebugMixin
from onetrainer.modules.modelSetup.mixin.ModelSetupDiffusionLossMixin import ModelSetupDiffusionLossMixin
from onetrainer.modules.modelSetup.mixin.ModelSetupDiffusionNoiseMixin import ModelSetupDiffusionNoiseMixin
from onetrainer.modules.modelSetup.stableDiffusion.checkpointing_util import create_checkpointed_forward
...
from onetrainer.modules.util.dtype_util import create_autocast_context, disable_fp16_autocast_context
...
class BasePixArtAlphaSetup(
    ...
    def __init__(self, train_device: torch.device, temp_device: torch.device, debug_mode: bool):
        super(BasePixArtAlphaSetup, self).__init__(
        ...
        with model.text_encoder_autocast_context:
            ...
            prompt_embeds = final_layer_norm(
                text_encoder_output.hidden_states[-(
                    1 + config.text_encoder_layer_skip)]
            )
    ...
    def predict(
        ...
        with model.autocast_context:
            ...
            is_align_prop_step = config.align_prop and (
                rand.random() < config.align_prop_probability)
            ...
            if config.model_type.has_conditioning_image_input():
                scaled_latent_conditioning_image = batch['latent_conditioning_image'] * \
                    vae_scaling_factor

            latent_noise = self._create_noise(
                scaled_latent_image, config, generator)
            ...
            if is_align_prop_step and not deterministic:
                ...
                timestep_high = int(
                    config.align_prop_steps * config.max_noising_strength)
                timestep_low = \
                ...
                resolution = torch.tensor(
                    [height, width]).repeat(batch_size, 1)
                aspect_ratio = torch.tensor(
                    [float(height / width)]).repeat(batch_size, 1)
                resolution = resolution.to(
                    dtype=config.train_dtype.torch_dtype(), device=self.train_device)
                aspect_ratio = aspect_ratio.to(
                    dtype=config.train_dtype.torch_dtype(), device=self.train_device)
                added_cond_kwargs = {
                    "resolution": resolution, "aspect_ratio": aspect_ratio}

                truncate_timestep_index = config.align_prop_steps - \
                    rand.randint(timestep_low, timestep_high)

                checkpointed_transformer = create_checkpointed_forward(
                    model.transformer, self.train_device)
                ...
                for step in range(config.align_prop_steps):
                    ...
                    if config.model_type.has_mask_input() and config.model_type.has_conditioning_image_input():
                        latent_input = torch.concat(
                            [scaled_noisy_latent_image, batch['latent_mask'],
                                scaled_latent_conditioning_image], 1
                        )
                    else:
                    ...
                    predicted_latent_noise = checkpointed_transformer(
                        ...
                        encoder_hidden_states=text_encoder_output.to(
                            dtype=config.train_dtype.torch_dtype()),
                        encoder_attention_mask=text_encoder_attention_mask.to(
                            dtype=config.train_dtype.torch_dtype()),
                        timestep=timestep,
                    ...
                    negative_predicted_latent_noise = checkpointed_transformer(
                        latent_input.to(
                            dtype=config.train_dtype.torch_dtype()),
                        encoder_hidden_states=negative_text_encoder_output.to(
                            dtype=config.train_dtype.torch_dtype()),
                    ...
                    cfg_grad = (predicted_latent_noise -
                                negative_predicted_latent_noise)
                    cfg_predicted_latent_noise = negative_predicted_latent_noise + \
                        config.align_prop_cfg_scale * cfg_grad
                    cfg_predicted_latent_noise = cfg_predicted_latent_noise.chunk(2, dim=1)[
                        0]
                ...
                predicted_latent_image = predicted_latent_image.to(
                ...
                text_encoder_attention_mask = text_encoder_attention_mask.view(
                    batch_size, -1)
            ...
            if self.debug_mode:
                with torch.no_grad():
                    ...
                    if is_align_prop_step:
                        # noise
                        noise = model.vae.decode(
                            latent_noise / vae_scaling_factor).sample
                        ...
                        # image
                        image = model.vae.decode(
                            scaled_latent_image / vae_scaling_factor).sample
                        ...
                        # predicted noise
                        predicted_noise = model.vae.decode(
                            predicted_latent_noise / vae_scaling_factor).sample
                        ...
                        noisy_image = model.vae.decode(
                            noisy_latent_image).sample
                        ...
                        if config.model_type.has_conditioning_image_input():
                            ...
                            conditioning_image = conditioning_image.clamp(
                                -1, 1)
                            self._save_image(
                                conditioning_image,
                                config.debug_dir + "/training_batches",
