...
from onetrainer.modules.module.BaseImageCaptionModel import CaptionSample, BaseImageCaptionModel
...
class WDModel(BaseImageCaptionModel):
    ...
    def generate_caption(
        ...
        general_labels = [(self.tag_names[i], probs[i])
                          for i in self.general_indexes if probs[i] > 0.35]
        character_labels = [(self.tag_names[i], probs[i])
                            for i in self.character_indexes if probs[i] > 0.8]
