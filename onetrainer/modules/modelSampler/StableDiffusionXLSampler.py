...
class StableDiffusionXLSampler(BaseModelSampler):
    def __init__(
        ...
        super(StableDiffusionXLSampler, self).__init__(
            train_device, temp_device)
    ...
    def __sample_base(
            ...
            on_update_progress: Callable[[
                int, int], None] = lambda _, __: None,
    ) -> Image:
        ...
        if hasattr(text_encoder_1.config, "use_attention_mask") and text_encoder_1.config.use_attention_mask:
            tokens_1_attention_mask = tokenizer_1_output.attention_mask.to(
                self.train_device)
        else:
        ...
        if hasattr(text_encoder_2.config, "use_attention_mask") and text_encoder_2.config.use_attention_mask:
            tokens_2_attention_mask = tokenizer_2_output.attention_mask.to(
                self.train_device)
        else:
        ...
        negative_tokens_1 = negative_tokenizer_1_output.input_ids.to(
            self.train_device)
        if hasattr(text_encoder_1.config, "use_attention_mask") and text_encoder_1.config.use_attention_mask:
            negative_tokens_1_attention_mask = negative_tokenizer_1_output.attention_mask.to(
                self.train_device)
        else:
        ...
        negative_tokens_2 = negative_tokenizer_2_output.input_ids.to(
            self.train_device)
        if hasattr(text_encoder_2.config, "use_attention_mask") and text_encoder_2.config.use_attention_mask:
            negative_tokens_2_attention_mask = negative_tokenizer_2_output.attention_mask.to(
                self.train_device)
        else:
        ...
        with torch.autocast(self.train_device.type):
            ...
            negative_text_encoder_1_output = \
                negative_text_encoder_1_output.hidden_states[-(
                    2 + text_encoder_layer_skip)]
            ...
            negative_text_encoder_2_output = \
                negative_text_encoder_2_output.hidden_states[-(
                    2 + text_encoder_layer_skip)]

            negative_prompt_embedding = torch.concat(
                [negative_text_encoder_1_output,
                    negative_text_encoder_2_output], dim=-1
            )

        combined_prompt_embedding = torch.cat(
            [negative_prompt_embedding, prompt_embedding])
        ...
        noise_scheduler.set_timesteps(
            diffusion_steps, device=self.train_device)
        ...
        latent_image = torch.randn(
            size=(1, num_channels_latents, height //
        ...
        for i, timestep in enumerate(tqdm(timesteps, desc="sampling")):
            ...
            if cfg_rescale > 0.0:
                ...
                std_positive = noise_pred_positive.std(
                    dim=list(range(1, noise_pred_positive.ndim)), keepdim=True)
                std_pred = noise_pred.std(
                    dim=list(range(1, noise_pred.ndim)), keepdim=True)
                noise_pred_rescaled = noise_pred * (std_positive / std_pred)
                noise_pred = (
                    cfg_rescale * noise_pred_rescaled +
                    (1 - cfg_rescale) * noise_pred
                )
        ...
        image = vae.decode(
            latent_image / vae.config.scaling_factor, return_dict=False)[0]
    ...
    def __sample_inpainting(
        ...
        conditioning_image = torch.zeros(
            (1, 3, height, width), dtype=torch.float32, device=self.train_device)
        conditioning_image = conditioning_image
        latent_conditioning_image = vae.encode(
            conditioning_image).latent_dist.mode() * vae.config.scaling_factor
        latent_mask = torch.ones(
            size=(
                1, 1, latent_conditioning_image.shape[2], latent_conditioning_image.shape[3]),
            dtype=torch.float32,
            device=self.train_device
        )
        self.model.vae_to(self.temp_device)
