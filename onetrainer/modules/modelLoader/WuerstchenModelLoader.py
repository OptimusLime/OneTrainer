...
from onetrainer.modules.model.WuerstchenModel import WuerstchenModel, WuerstchenEfficientNetEncoder
...
from onetrainer.modules.util.convert.convert_stable_cascade_ckpt_to_diffusers import convert_stable_cascade_ckpt_to_diffusers
...
class WuerstchenModelLoader(BaseModelLoader, ModelLoaderModelSpecMixin):
    ...
    def __load_internal(
        ...
        # optimizer
        try:
            model.optimizer_state_dict = torch.load(os.path.join(
                prior_model_name, "optimizer", "optimizer.pt"))
        ...
        try:
            model.ema_state_dict = torch.load(
                os.path.join(prior_model_name, "ema", "ema.pt"))
        except FileNotFoundError:
            pass
    ...
    def __load(
        ...
        if model_type.is_wuerstchen_v2():
            ...
            effnet_encoder.load_state_dict(
                load_file(effnet_encoder_model_name))
        ...
        if model_type.is_wuerstchen_v2():
            ...
            if prior_prior_model_name:
                ...
                prior_prior.load_state_dict(convert_stable_cascade_ckpt_to_diffusers(
                    load_file(prior_prior_model_name)))
        ...
        if model_type.is_wuerstchen_v2():
            ...
            prior_text_encoder.text_model.embeddings.to(
                dtype=weight_dtypes.text_encoder.torch_dtype(False))
        elif model_type.is_stable_cascade():
    ...
    def load(
        ...
        for stacktrace in stacktraces:
            print(stacktrace)
        raise Exception(
            "could not load model: " + prior_model_name + ", " +
            effnet_encoder_model_name + ", " + decoder_model_name
        )
