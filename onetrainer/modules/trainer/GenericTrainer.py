...
from onetrainer.modules.trainer.BaseTrainer import BaseTrainer
from onetrainer.modules.util import path_util, create
...
from onetrainer.modules.util.dtype_util import enable_grad_scaling
...
from onetrainer.modules.util.time_util import get_string_timestamp
...
class GenericTrainer(BaseTrainer):
    ...
    def __init__(self, config: TrainConfig, callbacks: TrainCallbacks, commands: TrainCommands):
        ...
        self.tensorboard = SummaryWriter(os.path.join(
            tensorboard_log_dir, get_string_timestamp()))
    ...
    def start(self):
        ...
        if self.config.continue_last_backup:
            ...
            if last_backup_path:
                ...
                print(
                    f"Continuing training from backup '{last_backup_path}'...")
            else:
        ...
        self.parameters = list(
            self.model_setup.create_parameters(self.model, self.config))
            ...
            if self.config.samples is not None:
                ...
                with open(self.config.sample_definition_file_name, 'r') as f:
                    ...
                    for i in range(len(samples)):
                        samples[i] = SampleConfig.default_values(
                        ).from_dict(samples[i])
    ...
    def backup(self, train_progress: TrainProgress):
        ...
        backup_path = os.path.join(
            self.config.workspace_dir, "backup", backup_name)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(train_progress.epoch, self.config.epochs, 1), desc="epoch"):
            ...
            current_epoch_length = len(
                self.data_loader.get_data_loader()) + train_progress.epoch_step
                ...
                if self.__is_update_step(train_progress):
                    ...
                    self.tensorboard.add_scalar(
                        "learning_rate", lr_scheduler.get_last_lr()[
                            0], train_progress.global_step
                    )
                    self.tensorboard.add_scalar(
                        "loss", accumulated_loss, train_progress.global_step)
                    ...
                    self.tensorboard.add_scalar(
                        "smooth loss", ema_loss, train_progress.global_step)
                    accumulated_loss = 0.0
