...
from onetrainer.modules.modelSetup.stableDiffusion.checkpointing_util import enable_checkpointing_for_clip_encoder_layers, \
...
class BaseWuerstchenSetup(
        ...
        if config.gradient_checkpointing:
            if model.model_type.is_wuerstchen_v2():
                ...
                enable_checkpointing_for_clip_encoder_layers(
                    model.prior_text_encoder, self.train_device)
            elif model.model_type.is_stable_cascade():
                enable_checkpointing_for_stable_cascade_blocks(
                    model.prior_prior, self.train_device)
                enable_checkpointing_for_clip_encoder_layers(
                    model.prior_text_encoder, self.train_device)
    ...
    def __alpha_cumprod(
        ...
        alpha_cumprod = torch.cos(
            (timesteps + s) / (1 + s) * torch.pi * 0.5) ** 2 / init_alpha_cumprod
    ...
    def predict(
        ...
        with model.autocast_context:
            ...
            if config.text_encoder.train or config.training_method == TrainingMethod.EMBEDDING:
                ...
                if model.model_type.is_stable_cascade():
                    text_embedding = text_encoder_output.hidden_states[-(
                        1 + config.text_encoder_layer_skip)]
                    if model.model_type.is_stable_cascade():
                        pooled_text_text_embedding = text_encoder_output.text_embeds.unsqueeze(
                            1)
            else:
                text_embedding = batch['text_encoder_hidden_state']
                if model.model_type.is_stable_cascade():
                    pooled_text_text_embedding = batch['pooled_text_encoder_output'].unsqueeze(
                        1)
            ...
            model_output_data = {
                ...
                # the DDPMWuerstchenScheduler only supports eps prediction
            ...
            if config.debug_mode:
                with torch.no_grad():
                    self._save_text(
                        self._decode_tokens(
                            batch['tokens'], model.prior_tokenizer),
                    ...
                    # noise
                    self._save_image(
                        self._project_latent_to_image(
                            latent_noise).clamp(-1, 1),
                        config.debug_dir + "/training_batches",
                        "1-noise",
                        train_progress.global_step
                    )
                    ...
                    self._save_image(
                        self._project_latent_to_image(
                            predicted_latent_noise).clamp(-1, 1),
                        config.debug_dir + "/training_batches",
                        "2-predicted_noise",
                        train_progress.global_step
                    )
                    ...
                    self._save_image(
                        self._project_latent_to_image(
                            scaled_noisy_latent_image).clamp(-1, 1),
                        config.debug_dir + "/training_batches",
                        "3-noisy_image",
                        train_progress.global_step
                    )
                    ...
                    # predicted image
                    alpha_cumprod = self.__alpha_cumprod(
                        timestep, latent_noise.dim())
                    ...
                    self._save_image(
                        self._project_latent_to_image(
                            scaled_predicted_latent_image).clamp(-1, 1),
                        config.debug_dir + "/training_batches",
                        "4-predicted_image",
    ...
    def calculate_loss(
        ...
        if config.min_snr_gamma:
            ...
            gamma = 1.0
            alpha_cumprod = self.__alpha_cumprod(
                data['timestep'], losses.dim())
            p2_loss_weight = (k + alpha_cumprod /
                              (1 - alpha_cumprod)) ** -gamma
