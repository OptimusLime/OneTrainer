...
class StableDiffusionFineTuneVaeDataLoader(BaseDataLoader):
    ...
    def __load_input_modules(self, config: TrainConfig) -> list:
        load_image = LoadImage(
            path_in_name='image_path', image_out_name='image', range_min=-1.0, range_max=1.0)
        load_mask = LoadImage(
            path_in_name='mask_path', image_out_name='latent_mask', range_min=0, range_max=1, channels=1)
    ...
    def __cache_modules(self, config: TrainConfig, model: StableDiffusionModel):
        ...
        def before_cache_fun():
            self._setup_cache_device(
                model, self.train_device, self.temp_device, config)

        disk_cache = DiskCache(cache_dir=config.cache_dir, split_names=split_names, aggregate_names=aggregate_names, variations_in_name='concept.image_variations', repeats_in_name='concept.repeats',
        ...
        ram_cache = RamCache(cache_names=split_names + aggregate_names, repeats_in_name='concept.repeats', variations_group_in_name=[
                             'concept.path', 'concept.seed', 'concept.include_subdirectories', 'concept.image'], group_enabled_in_name='concept.enabled', before_cache_fun=before_cache_fun)
    ...
    def __debug_modules(self, config: TrainConfig, model: StableDiffusionModel):
        ...
        save_mask = SaveImage(image_in_name='latent_mask', original_path_in_name='image_path',
