...
from onetrainer.modules.ui.ConfigList import ConfigList
from onetrainer.modules.ui.SampleParamsWindow import SampleParamsWindow
...
class SampleWidget(ctk.CTkFrame):
    def __init__(self, master, element, i, open_command, remove_command, clone_command, save_command):
        ...
        # enabled
        self.enabled_switch = components.switch(
            self, 0, 2, self.ui_state, "enabled", self.__switch_enabled)
        ...
        self.height_entry = components.entry(
            self, 0, 6, self.ui_state, "height")
        ...
        self.prompt_entry = components.entry(
            self, 0, 10, self.ui_state, "prompt")
        self.prompt_entry.bind('<FocusOut>', lambda _: save_command())

        # button
        self.button = components.icon_button(
            self, 0, 11, "...", lambda: open_command(self.i, self.ui_state))
