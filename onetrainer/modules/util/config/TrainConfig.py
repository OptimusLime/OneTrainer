...
from onetrainer.modules.util.enum.AlignPropLoss import AlignPropLoss
from onetrainer.modules.util.enum.AttentionMechanism import AttentionMechanism
from onetrainer.modules.util.enum.ConfigPart import ConfigPart
from onetrainer.modules.util.enum.DataType import DataType
from onetrainer.modules.util.enum.EMAMode import EMAMode
from onetrainer.modules.util.enum.ImageFormat import ImageFormat
from onetrainer.modules.util.enum.LearningRateScaler import LearningRateScaler
from onetrainer.modules.util.enum.LearningRateScheduler import LearningRateScheduler
from onetrainer.modules.util.enum.LossScaler import LossScaler
...
from onetrainer.modules.util.enum.TimeUnit import TimeUnit
...
class TrainOptimizerConfig(BaseConfig):
    ...
    def default_values():
        ...
        # Disabled, because it uses too much VRAM
        data.append(("foreach", False, bool, True))
...
class TrainModelPartConfig(BaseConfig):
    ...
    def default_values():
        ...
        data.append(("stop_training_after_unit",
                    TimeUnit.NEVER, TimeUnit, False))
...
class TrainConfig(BaseConfig):
    ...
    def to_pack_dict(self) -> dict:
        ...
        with open(config.concept_file_name, 'r') as f:
            ...
            for i in range(len(concepts)):
                concepts[i] = ConceptConfig.default_values(
                ).from_dict(concepts[i])
    ...
    def default_values() -> 'TrainConfig':
        ...
        # general settings
        data.append(
            ("training_method", TrainingMethod.FINE_TUNE, TrainingMethod, False))
        data.append(
            ("model_type", ModelType.STABLE_DIFFUSION_15, ModelType, False))
        ...
        data.append(("include_train_config",
                    ConfigPart.NONE, ConfigPart, False))

        # model settings
        data.append(
            ("base_model_name", "runwayml/stable-diffusion-v1-5", str, False))
        data.append(("weight_dtype", DataType.FLOAT_32, DataType, False))
        data.append(("output_dtype", DataType.FLOAT_32, DataType, False))
        data.append(
            ("output_model_format", ModelFormat.SAFETENSORS, ModelFormat, False))
        data.append(("output_model_destination",
                    "models/model.safetensors", str, False))
        data.append(("gradient_checkpointing", True, bool, False))

        # data settings
        data.append(
            ("concept_file_name", "training_concepts/concepts.json", str, False))
        ...
        # training settings
        data.append(("learning_rate_scheduler",
                    LearningRateScheduler.CONSTANT, LearningRateScheduler, False))
        ...
        data.append(("fallback_train_dtype",
                    DataType.BFLOAT_16, DataType, False))
        data.append(("only_cache", False, bool, False))
        data.append(("resolution", "512", str, False))
        data.append(("attention_mechanism",
                    AttentionMechanism.XFORMERS, AttentionMechanism, False))
        data.append(("align_prop", False, bool, False))
        data.append(("align_prop_probability", 0.1, float, False))
        data.append(
            ("align_prop_loss", AlignPropLoss.AESTHETIC, AlignPropLoss, False))
        ...
        data.append(("learning_rate_scaler",
                    LearningRateScaler.NONE, LearningRateScaler, False))

        # noise
        data.append(("offset_noise_weight", 0.0, float, False))
        data.append(("perturbation_noise_weight", 0.0, float, False))
        data.append(
            ("rescale_noise_scheduler_to_zero_terminal_snr", False, bool, False))
        ...
        data.append(("text_encoder", text_encoder,
                    TrainModelPartConfig, False))
        ...
        data.append(("text_encoder_2", text_encoder_2,
                    TrainModelPartConfig, False))
        ...
        data.append(("effnet_encoder", effnet_encoder,
                    TrainModelPartConfig, False))
        ...
        data.append(("decoder_text_encoder", decoder_text_encoder,
                    TrainModelPartConfig, False))

        # decoder vqgan
        decoder_vqgan = TrainModelPartConfig.default_values()
        decoder_vqgan.weight_dtype = DataType.NONE
        data.append(("decoder_vqgan", decoder_vqgan,
                    TrainModelPartConfig, False))
        ...
        data.append(("embeddings", [
                    TrainEmbeddingConfig.default_values()], list[TrainEmbeddingConfig], False))
        data.append(("embedding_weight_dtype",
                    DataType.FLOAT_32, DataType, False))
        ...
        # optimizer
        data.append(("optimizer", TrainOptimizerConfig.default_values(),
                    TrainOptimizerConfig, False))
        data.append(("optimizer_defaults", {},
                    dict[str, TrainOptimizerConfig], False))

        # sample settings
        data.append(("sample_definition_file_name",
                    "training_samples/samples.json", str, False))
        ...
        data.append(
            ("sample_image_format", ImageFormat.JPG, ImageFormat, False))
