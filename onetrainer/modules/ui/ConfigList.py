...
from onetrainer.modules.util.ui import components, dialogs
...
class ConfigList(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.current_config = []
        self.widgets = []
        self.__load_current_config(
            getattr(self.train_config, self.element_attr_name))

        self.__create_configs_dropdown()

        components.icon_button(self.top_frame, 0, 2,
                               "add config", self.__add_config)

        components.icon_button(self.top_frame, 0, 3,
                               add_button_text, self.__add_element)
    ...
    def __create_element_list(self):
        ...
        self.element_list = ctk.CTkScrollableFrame(
    ...
    def __add_config(self):
        dialogs.StringInputDialog(
            self.master, "name", "Name", self.__create_config)
