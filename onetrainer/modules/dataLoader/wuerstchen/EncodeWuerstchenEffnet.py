...
from onetrainer.modules.model.WuerstchenModel import WuerstchenEfficientNetEncoder
...
class EncodeWuerstchenEffnet(
    ...
    def get_item(self, variation: int, index: int, requested_name: str = None) -> dict:
        ...
        image = image.to(device=self.effnet_encoder.device,
                         dtype=self.effnet_encoder.dtype)

        with self.autocast_context:
            image_embeddings = self.effnet_encoder(
                image.unsqueeze(0)).squeeze()
