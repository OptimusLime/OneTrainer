...
from onetrainer.modules.module.BaseImageMaskModel import BaseImageMaskModel, MaskSample
...
class BaseRembgModel(BaseImageMaskModel):
    def __init__(
        ...
        self.smoothing_kernel = self.__create_average_kernel(
            self.smoothing_kernel_radius)

        self.expand_kernel_radius = None
        self.expand_kernel = self.__create_average_kernel(
            self.expand_kernel_radius)
    ...
    def __load_model(self) -> onnxruntime.InferenceSession:
        ...
        if self.device.type == 'cpu':
            ...
            provider = "CUDAExecutionProvider" if "CUDAExecutionProvider" in onnxruntime.get_available_providers(
    ...
    def __create_average_kernel(self, kernel_radius: Optional[int]):
        ...
        kernel_weights = torch.ones(
            1, 1, kernel_size, kernel_size) / (kernel_size * kernel_size)
        kernel = nn.Conv2d(
    ...
    def mask_image(
        ...
        predicted_mask = self.__process_mask(
            output, mask_sample.height, mask_sample.width, threshold)
