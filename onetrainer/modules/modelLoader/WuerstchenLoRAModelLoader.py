...
from onetrainer.modules.util.convert.convert_stable_cascade_lora_ckpt_to_diffusers import \
...
class WuerstchenLoRAModelLoader(BaseModelLoader, ModelLoaderModelSpecMixin, ModelLoaderLoRAMixin):
    ...
    def __load_safetensors(
        ...
        if model.model_type.is_stable_cascade():
            state_dict = convert_stable_cascade_lora_ckpt_to_diffusers(
                state_dict)
    ...
    def __load_internal(
        ...
        try:
            with open(os.path.join(lora_name, "model_spec.json"), "r") as model_spec_file:
                model.model_spec = ModelSpec.from_dict(
                    json.load(model_spec_file))
        except:
            pass
