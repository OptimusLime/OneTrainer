...
from onetrainer.modules.util.ui import components
from onetrainer.modules.util.ui.UIState import UIState
...
class ConceptWindow(ctk.CTkToplevel):
    ...
    def __general_tab(self, master, concept: ConceptConfig):
        ...
        prompt_path_entry = components.file_entry(
            master, 3, 2, self.text_ui_state, "prompt_path")
        ...
        components.switch(master, 4, 1, self.ui_state,
    ...
    def __image_augmentation_tab(self, master):
        ...
        components.switch(master, 1, 1, self.image_ui_state,
                          "enable_crop_jitter")

        # random flip
        components.label(master, 2, 0, "Random Flip",
                         tooltip="Randomly flip the sample during training")
        components.switch(master, 2, 1, self.image_ui_state,
                          "enable_random_flip")
        components.switch(master, 2, 2, self.image_ui_state,
                          "enable_fixed_flip")

        # random rotation
        components.label(master, 3, 0, "Random Rotation",
                         tooltip="Randomly rotates the sample during training")
        components.switch(master, 3, 1, self.image_ui_state,
                          "enable_random_rotate")
        components.switch(master, 3, 2, self.image_ui_state,
                          "enable_fixed_rotate")
        components.entry(master, 3, 3, self.image_ui_state,
        ...
        components.switch(master, 4, 1, self.image_ui_state,
                          "enable_random_brightness")
        components.switch(master, 4, 2, self.image_ui_state,
                          "enable_fixed_brightness")
        components.entry(master, 4, 3, self.image_ui_state,
                         "random_brightness_max_strength")

        # random contrast
        components.label(master, 5, 0, "Random Contrast",
                         tooltip="Randomly adjusts the contrast of the sample during training")
        components.switch(master, 5, 1, self.image_ui_state,
                          "enable_random_contrast")
        components.switch(master, 5, 2, self.image_ui_state,
                          "enable_fixed_contrast")
        components.entry(master, 5, 3, self.image_ui_state,
        ...
        components.switch(master, 6, 1, self.image_ui_state,
                          "enable_random_saturation")
        components.switch(master, 6, 2, self.image_ui_state,
                          "enable_fixed_saturation")
        components.entry(master, 6, 3, self.image_ui_state,
                         "random_saturation_max_strength")

        # random hue
        components.label(master, 7, 0, "Random Hue",
                         tooltip="Randomly adjusts the hue of the sample during training")
        components.switch(master, 7, 1, self.image_ui_state,
                          "enable_random_hue")
        components.switch(master, 7, 2, self.image_ui_state,
                          "enable_fixed_hue")
        components.entry(master, 7, 3, self.image_ui_state,
        ...
        components.switch(master, 8, 2, self.image_ui_state,
                          "enable_resolution_override")
        components.entry(master, 8, 3, self.image_ui_state,
                         "resolution_override")
        ...
        image_label = ctk.CTkLabel(
            master=master, text="", image=self.image, height=150, width=150)
        image_label.grid(row=0, column=4, rowspan=6)

        # refresh preview
        components.button(master, 6, 4, "Update Preview",
                          command=self.__update_image_preview)
    ...
    def __text_augmentation_tab(self, master):
        ...
        components.switch(master, 0, 1, self.text_ui_state,
                          "enable_tag_shuffling")
    ...
    def __get_preview_image(self):
        ...
        if os.path.isdir(self.concept.path):
            for path in os.scandir(self.concept.path):
                ...
                if path.is_file() \
                    ...
                    preview_path = path_util.canonical_join(
                        self.concept.path, path.name)
                    break
        ...
        if self.concept.image.enable_random_rotate or self.concept.image.enable_fixed_rotate:
            ...
            if self.concept.image.enable_random_rotate:
                ...
                angle = max_angle
            image_tensor = functional.rotate(
                image_tensor, angle, interpolation=InterpolationMode.BILINEAR)
        ...
        if self.concept.image.enable_random_hue or self.concept.image.enable_fixed_hue:
            ...
            if self.concept.image.enable_random_hue:
                strength = rand.uniform(-max_strength *
                                        0.5, max_strength * 0.5)
